...
Tenant shall be responsible for the first $350 of all maintenance and repair costs per occurrence. Overnight guests remaining longer than 3 nights are strictly prohibited under penalty of $500 fine.`;
    ...
    <div style="display:flex; flex-direction:column; gap:1.25rem;">
      <div class="glass-panel" style="padding:1.25rem; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem;">
        <div>
          ...
          <p id="comp-summary" style="font-size:0.86rem; color:var(--text-secondary);"></p>
        </div>

        <div style="display:flex; gap:0.75rem;">
          <div class="stat-card" style="padding:0.5rem 1rem;">
            <span id="comp-added-count" style="font-weight:800; color:var(--risk-high);">0</span>
            <span style="font-size:0.72rem; color:var(--text-muted);">New Restrictions</span>
          </div>
          <div class="stat-card" style="padding:0.5rem 1rem;">
            <span id="comp-removed-count" style="font-weight:800; color:var(--risk-medium);">0</span>
      ...
      <div class="glass-panel" style="padding:1.35rem;">
        <h4 id="comp-risk-shift" style="font-size:0.95rem; font-weight:800; margin-bottom:0.85rem; display:flex; align-items:center; gap:0.4rem;">
        ...
        <div id="comp-diff-list" style="display:flex; flex-direction:column; gap:0.6rem;"></div>
      </div>
    </div>
  `;
  ...
  const summaryEl = container.querySelector('#comp-summary');
  const addedCountEl = container.querySelector('#comp-added-count');
  const removedCountEl = container.querySelector('#comp-removed-count');
  const riskShiftEl = container.querySelector('#comp-risk-shift');
  const diffListEl = container.querySelector('#comp-diff-list');
  ...
  function updateDiffView() {
    ...
    if (summaryEl) summaryEl.textContent = diffResult.summary;
    if (addedCountEl) addedCountEl.textContent = diffResult.addedCount;
    if (removedCountEl) removedCountEl.textContent = diffResult.removedCount;
    if (riskShiftEl) {
      riskShiftEl.innerHTML = `
        <i data-lucide="shield-alert" style="width:16px; height:16px; color:var(--risk-high);" aria-hidden="true"></i>
        Differential Risk Analysis Highlights (${diffResult.riskShift})
      `;
      if (window.lucide) window.lucide.createIcons({ root: riskShiftEl });
    }

    if (diffListEl) {
      const addedHtml = diffResult.added.map((item) => `
      ...
      const removedHtml = diffResult.removed.map((item) => `
      ...
      diffListEl.innerHTML = (addedHtml + removedHtml) || `
        <div style="text-align:center; padding:1rem; color:var(--text-muted); font-size:0.85rem;">
          No differential discrepancies identified. Documents are identical.
        </div>
      `;
    }
  }
